# nf-core/mofapipeline: Configuration for other clusters
...
### Docker
Docker is a great way to run nf-core/mofapipeline, as it manages all software installations and allows the pipeline to be run in an identical software environment across a range of systems.

Nextflow has [excellent integration](https://www.nextflow.io/docs/latest/docker.html) with Docker, and beyond installing the two tools, not much else is required - nextflow will automatically fetch the [nfcore/mofapipeline](https://hub.docker.com/r/nfcore/mofapipeline/) image that we have created and is hosted at dockerhub at run time.

To add docker support to your own config file, add the following:

```nextflow
docker.enabled = true
process.container = "nfcore/mofapipeline"
```
...
process.container = "shub://nf-core/mofapipeline"
```
...
```nextflow
singularity.enabled = true
process.container = "/path/to/nf-core-mofapipeline.simg"
```
